import { Component, ElementRef, AfterViewInit, forwardRef, Optional, Inject, input, output, viewChild, OnDestroy, effect, Provider } from '@angular/core';
...
import { 
  placeholder, EditorViewConfig, lineNumbers, highlightActiveLineGutter, 
  highlightSpecialChars, drawSelection, dropCursor, rectangularSelection, 
  crosshairCursor, highlightActiveLine, keymap, KeyBinding, Decoration, 
  DecorationSet, ViewPlugin, ViewUpdate 
  ...
  buildDecorations(view: EditorView) {
    ...
    for (const { from, to } of view.visibleRanges) {
      ...
      while ((match = tagRegex.exec(text)) !== null) {
        decorations.push(Decoration.mark({ class: "cm-tag-x-" + match[1] })
      ...
      while ((match = variableRegex.exec(text)) !== null) {
        ...
        try {
          ...
          decorations.push(Decoration.mark({ class: "cm-js-expression" })
      ...
      while ((match = rekaKeywordRegex.exec(text)) !== null) {
        decorations.push(Decoration.mark({ class: "cm-reka-key" })
          .range(from + match.index, from + match.index + match[0].length));
      }

      attrRegex.lastIndex = 0;
      while ((match = attrRegex.exec(text)) !== null) {
        decorations.push(Decoration.mark({ class: "cm-attr-x-" + match[1] })
          .range(from + match.index, from + match.index + match[0].length));
      }
    }
...
export class NgCmComponent extends ElementBase<any> implements AfterViewInit, OnDestroy {
  ...
  options = input<any>(); 
  lang = input<string>('javascript'); 
  editorStyle = input<any>(); 
  linenumber = input<boolean>(true);
  readOnly = input<boolean>(false);
  lambda = input<boolean>(false);
  hideControl = input<boolean>(false);
  ...
  override writeValue(value: string): void {
    ...
    this.evaluateASTChecks(safeValue);
  }
  ...
  ngAfterViewInit(): void {
    ...
    const completionPack: Record<string, any[]> = {
    ...
    const currentLang = this.lang() || 'javascript';
    ...
    const ext = [
      ...
      ...(langPack[currentLang] || []),
      ...(completionPack[currentLang] || [])
    ];
  ...
  myJsCompletions = (context: CompletionContext) => {
    ...
    if (this.dontComplete.includes(inner.name)) return null;
  ...
  myLambdaCompletions = (context: CompletionContext) => {
    ...
    return {
      ...
      options: this.processSnippet([
        ...
        { label: '_param._body', type: "method", apply: "_param._body", detail: "Get request body from POST request" },
  ...
  myHtmlCompletions = (context: CompletionContext) => {
    ...
    return {
      from: word ? word.from + word.text.search(/\S|$/) : context.pos,
      options: this.processSnippet([
        ...defaultList,
        ...this.extraAutoComplete()
      ]),
  ...
  checkCode = (code: string) => {
    try {
      const validatorMap: Record<string, (c: string) => void> = {
        "javascript": this.isValidJs,
        "json": this.isValidJson,
        "lambda": this.isValidJs,
        "html": this.isValidHTML
      };
  ...
  isAssignmentExpression(codeString: string): boolean {
    ...
    try {
      ...
      if (String(cursor.name) === "ExpressionStatement") {
        if (cursor.firstChild()) {
          return String(cursor.name) === "AssignmentExpression";
        }
      }
      return false;
    } catch {
